...
func checkPairOk(a, b bool) {
	assert.Ok(a && b, "a and b") // want `TS-S07: this assert\.Ok checks two things joined by &&`
}

// checkTripleOk joins three checks with &&.
func checkTripleOk(a, b, c bool) {
	assert.Ok(a && b && c, "all three") // want `TS-S07: this assert\.Ok checks two things joined by &&`
}

// checkInvariant joins two checks with && inside assert.Invariant's second
// argument.
func checkInvariant(id string, a, b bool) {
	assert.Invariant(id, a && b) // want `TS-S07: this assert\.Invariant checks two things joined by &&`
}
