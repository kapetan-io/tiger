...
func drain(pending []int) int {
	drained := 0
	for len(pending) > 0 { // want `TS-V01: synthesized variant for drain — //tiger:variant len\(pending\)`
		pending = pending[1:]
		drained++
	}
	return drained
...
func twoPointer(arr []int, target int) bool {
	...
	for low < high { // want `TS-V01: synthesized variant for twoPointer — //tiger:variant high - low`
...
func countdown(i int) int {
	ticks := 0
	for i > 0 { // want `TS-V01: synthesized variant for countdown — //tiger:variant i`
		i--
		ticks++
	}
	return ticks
...
func indexWalk(i, n int) int {
	sum := 0
	for i < n { // want `TS-V01: synthesized variant for indexWalk — //tiger:variant n - i`
		sum += i
		i += 2
	}
	return sum
